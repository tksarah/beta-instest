...
const teacherSessionMaxAgeSec = 60 * 60 * 24 * 14;
...
function setTeacherSessionCookie(res, token, maxAgeSec){
  ...
  const cookie = `teacher_session=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${safeAge}${cookieSecurityAttribute()}`;
  res.setHeader('Set-Cookie', cookie);
}

function clearTeacherSessionCookie(res){
  res.setHeader('Set-Cookie', `teacher_session=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0${cookieSecurityAttribute()}`);
}
...
function setOauthStateCookie(res, state){
  addSetCookie(res, `google_oauth_state=${encodeURIComponent(state)}; Path=/api/auth/google; HttpOnly; SameSite=Lax; Max-Age=${oauthStateMaxAgeSec}${cookieSecurityAttribute()}`);
}

function clearOauthStateCookie(res){
  addSetCookie(res, `google_oauth_state=; Path=/api/auth/google; HttpOnly; SameSite=Lax; Max-Age=0${cookieSecurityAttribute()}`);
}

function normalizeLocalNextPath(value){
  const next = typeof value === 'string' ? value.trim() : '';
  if(next && next.startsWith('/') && !next.startsWith('//') && !/^\/api(?:\/|$)/i.test(next)){
    return next;
  }
  return '/app.html';
}

async function createTeacherSession(res, teacherId){
  const token = crypto.randomBytes(32).toString('hex');
  const nowIso = new Date().toISOString();
  const expiresIso = new Date(Date.now() + teacherSessionMaxAgeSec * 1000).toISOString();
  await dbRunAsync(
    'INSERT INTO teacher_sessions (token, teacher_id, created_at, last_seen_at, expires_at) VALUES (?,?,?,?,?)',
    [token, teacherId, nowIso, nowIso, expiresIso]
  );
  setTeacherSessionCookie(res, token, teacherSessionMaxAgeSec);
  return token;
}
...
function makeOAuthPlaceholderPasswordHash(){
  return makePasswordHash(crypto.randomBytes(32).toString('hex'));
}

async function findOrCreateGoogleTeacher(profile){
  const nowIso = new Date().toISOString();
  const existingBySub = await dbGetAsync('SELECT * FROM teachers WHERE google_sub=?', [profile.sub]);
  if(existingBySub){
    await dbRunAsync(
      'UPDATE teachers SET email=?, display_name=COALESCE(NULLIF(display_name, \'\'), ?), avatar_url=?, auth_provider=?, active=1, last_login_at=? WHERE id=?',
      [profile.email, profile.displayName, profile.avatarUrl, 'google', nowIso, existingBySub.id]
    );
    return dbGetAsync('SELECT * FROM teachers WHERE id=?', [existingBySub.id]);
  }

  const existingByEmail = await dbGetAsync('SELECT * FROM teachers WHERE email=? OR lower(username)=?', [profile.email, profile.email]);
  if(existingByEmail){
    await dbRunAsync(
      'UPDATE teachers SET email=?, google_sub=?, display_name=COALESCE(NULLIF(display_name, \'\'), ?), avatar_url=?, auth_provider=?, active=1, plan=COALESCE(NULLIF(plan, \'\'), ?), last_login_at=? WHERE id=?',
      [profile.email, profile.sub, profile.displayName, profile.avatarUrl, 'google', 'free_beta', nowIso, existingByEmail.id]
    );
    return dbGetAsync('SELECT * FROM teachers WHERE id=?', [existingByEmail.id]);
  }

  const inserted = await dbRunAsync(
    'INSERT INTO teachers (username, email, google_sub, display_name, avatar_url, auth_provider, plan, password_hash, active, created_at, last_login_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
    [profile.email, profile.email, profile.sub, profile.displayName, profile.avatarUrl, 'google', 'free_beta', makeOAuthPlaceholderPasswordHash(), 1, nowIso, nowIso]
  );
...
async function getTeacherPlan(teacherId){
  const row = await dbGetAsync('SELECT plan FROM teachers WHERE id=?', [teacherId]);
...
async function getMonthlyAiUsage(teacherId){
  const row = await dbGetAsync('SELECT ai_generations FROM teacher_usage WHERE teacher_id=? AND usage_month=?', [teacherId, currentUsageMonth()]);
  return row ? Number(row.ai_generations || 0) : 0;
}
...
async function incrementMonthlyAiUsage(teacherId){
  const month = currentUsageMonth();
  const nowIso = new Date().toISOString();
  await dbRunAsync(
    `INSERT INTO teacher_usage (teacher_id, usage_month, ai_generations, updated_at)
     VALUES (?,?,1,?)
     ON CONFLICT(teacher_id, usage_month)
     DO UPDATE SET ai_generations=ai_generations + 1, updated_at=excluded.updated_at`,
    [teacherId, month, nowIso]
  );
}
...
app.use((req, res, next) => {
  ...
  db.get(
    'SELECT ts.token, ts.teacher_id, ts.expires_at, t.username, t.display_name, t.email, t.avatar_url, t.auth_provider, t.plan, t.active FROM teacher_sessions ts JOIN teachers t ON t.id=ts.teacher_id WHERE ts.token=?',
    [token],
    (err, row) => {
...
app.get('/api/auth/google/start', (req, res) => {
  try{
    ...
    const oauthClient = createGoogleOAuthClient();
    const nextPath = normalizeLocalNextPath(req.query.next);
    const state = crypto.randomBytes(24).toString('hex');
    const expiresAt = Date.now() + oauthStateMaxAgeSec * 1000;
    oauthStates.set(state, { nextPath, expiresAt });
...
app.get('/api/auth/google/callback', async (req, res) => {
  ...
  try{
    ...
    const teacher = await findOrCreateGoogleTeacher(profile);
    if(!teacher || teacher.active === 0){
      return res.status(403).json({ error: 'teacher_inactive' });
    }
    await createTeacherSession(res, teacher.id);
    res.redirect(stored.nextPath || '/app.html');
  }catch(err){
    res.status(err.statusCode || 500).json({ error: err.message || 'google_oauth_failed' });
  }
});
...
app.post('/api/teacher/login', (req, res) => {
  ...
  db.get('SELECT * FROM teachers WHERE username=? AND active=1', [u], (err, row) => {
    ...
    createTeacherSession(res, row.id)
      .then(() => {
        db.run('UPDATE teachers SET last_login_at=? WHERE id=?', [new Date().toISOString(), row.id], () => {
          res.json({ teacher: { id: row.id, username: row.username, display_name: row.display_name || '', email: row.email || '', auth_provider: row.auth_provider || 'password', plan: row.plan || 'free_beta' } });
        });
      })
...
app.post('/api/generate-questions', requireTeacher, async (req, res) => {
  ...
  try{
    await enforceMonthlyAiLimit(req.teacher.id);
    ...
    await incrementMonthlyAiUsage(req.teacher.id);
  ...
  findClass((err, cls) => {
    ...
    (async () => {
      try{
        ...
        const inserted = await dbRunAsync('INSERT INTO students (class_id, name, code) VALUES (?,?,?)', [cls.id, name, code]);
        const studentId = inserted.lastID;
        ...
        if(err2.statusCode === 403 && err2.limitName) return planLimitResponse(res, err2.limitName);
        res.status(err2.statusCode || 500).json({ error: err2.message });
      }
    })();
  });
});
